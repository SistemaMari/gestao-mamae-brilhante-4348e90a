...
export default function MeusCursosPage() {
  ...
  const acessarCurso = (curso: Curso) => {
    if (curso.link_eduzz) {
      ...
      toast.info(t('meusCursos.linkSoon'));
    }
  };
  ...
  return (
    <div className="container max-w-6xl py-8">
      <header className="mb-8">
        <h1 className="font-heading text-3xl font-bold text-foreground">{t('meusCursos.title')}</h1>
        <p className="mt-2 text-muted-foreground">
          {t('meusCursos.subtitle')}
        </p>
      </header>
      ...
      {!carregando && erro && (
        <Card className="mx-auto max-w-md">
          <CardContent className="flex flex-col items-center gap-4 py-10 text-center">
            ...
            <p className="text-foreground">{t('meusCursos.loadError')}</p>
            <Button onClick={fetchDados} variant="outline">{t('common.tryAgain')}</Button>
          </CardContent>
        </Card>
      )}
      ...
      {!carregando && !erro && !profissionalData?.plano_id && (
        <Card className="mx-auto max-w-md">
          <CardContent className="flex flex-col items-center gap-4 py-10 text-center">
            ...
            <p className="text-foreground">{t('meusCursos.noActivePlan')}</p>
            <Button onClick={() => navigate('/planos')} style={{ backgroundColor: '#7C4DBA' }}>
              {t('meusCursos.viewPlans')}
            </Button>
          </CardContent>
        </Card>
      )}
      ...
      {!carregando && !erro && plano && cursos.length === 0 && (
        <Card className="mx-auto max-w-md">
          <CardContent className="flex flex-col items-center gap-4 py-10 text-center">
            <GraduationCap className="h-10 w-10 text-muted-foreground" />
            <p className="text-foreground">{t('meusCursos.noCourses')}</p>
          </CardContent>
        </Card>
      )}
                ...
                <CardContent className="flex flex-col gap-4 p-6">
                  ...
                  {liberado ? (
                    ...
                    >
                      ...
                      {t('meusCursos.availableInPlan', { plano: nomePlanoNecessario })}
                    </span>
                  )}
