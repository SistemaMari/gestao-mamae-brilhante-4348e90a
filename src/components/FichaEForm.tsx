/**
 * 36E-B — Ficha E: Perfil glicêmico de 6 pontos SEM insulina (10 dias).
 *
 * Clone visual da Ficha B/D (mesma grade, metas e tooltips), porém:
 *   - 10 dias (não 15/7);
 *   - SEM checklist do Retorno 2;
 *   - Decisão local determinística (espelha 36E-A):
 *       ≥ 70% → manter_e (permanece na Ficha E, sem dose, retorno 7-10 dias);
 *       < 70% → insulina (peso capturado no result card → dose 0,5 UI/kg/dia → migra para Ficha B/D).
...
const IS_PRE_PRANDIAL: Record<Point6, boolean> = {
  jejum: false, pos_cafe: false, pre_almoco: true,
  pos_almoco: false, pre_jantar: true, pos_jantar: false,
};
...
export default function FichaEForm({
  ...
  const [grid, setGrid] = useState<Record<string, string>[]>(() => {
    if (editingConsulta?.grid_valores && editingConsulta.grid_valores.length > 0) {
      ...
      while (existing.length < DAYS.length) {
        existing.push(Object.fromEntries(POINTS_6.map(p => [p, ''])));
      }
      return existing;
    }
  ...
  const dataProximoRetorno = dataConsultaLocal
    ? format(addDays(dataConsultaLocal, RETORNO_DIAS), 'dd/MM/yyyy')
    : null;
  ...
  const handleKeyDown = (
    e: React.KeyboardEvent<HTMLInputElement>, dayIdx: number, colIdx: number,
  ) => {
    let nextDay = dayIdx;
    let nextCol = colIdx;
    if (e.key === 'Tab' || e.key === 'Enter') {
      e.preventDefault();
      nextCol++;
      if (nextCol >= POINTS_6.length) { nextCol = 0; nextDay++; }
    } else if (e.key === 'ArrowRight') nextCol = Math.min(colIdx + 1, POINTS_6.length - 1);
    else if (e.key === 'ArrowLeft') nextCol = Math.max(colIdx - 1, 0);
    else if (e.key === 'ArrowDown') nextDay = Math.min(dayIdx + 1, DAYS.length - 1);
    else if (e.key === 'ArrowUp') nextDay = Math.max(dayIdx - 1, 0);
    else return;
  ...
  const hasHighValues = useMemo(
    () => grid.some(row => POINTS_6.some(p => { const v = parseInt(row[p]); return v > 400; })),
    [grid],
  );
  const hasNegativeValues = useMemo(
    () => grid.some(row => POINTS_6.some(p => { const v = parseInt(row[p]); return !isNaN(v) && v < 0; })),
    [grid],
  );
  ...
  const handleSave = async () => {
    ...
    // Espelho da decisão do motor 36E-A
    const conduta_e: 'manter_e' | 'insulina' = isAdequado ? 'manter_e' : 'insulina';
    const proxima: 'ficha_e' | 'ficha_b' | 'ficha_d' =
      conduta_e === 'manter_e' ? 'ficha_e' : (igS <= 30 ? 'ficha_b' : 'ficha_d');
    const decisao = isAdequado ? 'controle_adequado' : 'controle_inadequado';

    // Critério 7 — status permanece sempre 'dmg_confirmado' (acompanhamento ativo)
    const newStatus = 'dmg_confirmado';
    ...
    if (isPreview) {
      ...
      const consultaData: PreviewConsulta = {
        ...
        retorno_dias: conduta_e === 'manter_e' ? RETORNO_DIAS : null,
        data_proximo_retorno_formatted: conduta_e === 'manter_e' ? dataProximoRetorno : null,
        grid_valores: grid,
        decisao,
        data_inicio: dataInicio,
        data_fim: dataFim,
        proxima_ficha_recomendada: proxima,
        conduta_gerada: conduta_e,
        conduta_e,
      };
  ...
  return (
    <div className="space-y-5">
      ...
      <CamposPendentesBanner pendentes={camposPendentes} ativo={statusFichaLocal === 'rascunho'} />
