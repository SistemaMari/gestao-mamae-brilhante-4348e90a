-- ============================================================
-- Fix: metricas_diagnosticos_admin contava cenários pelo formato
-- PREFIXADO ('cenario_1'...) mas consultas.cenario_clinico guarda o
-- código CRU ('1','6','6B','7','8'...). Resultado: DMG/cenário 7 = 0 no
-- painel admin, mesmo havendo 141 consultas de DMG. Aqui só corrigimos os
-- LITERAIS da função (dados intactos). Re-emite a def atual de 005134.
-- ⚠️ Aplicar via chat do Lovable OU à mão no Supabase.
-- ============================================================

-- Função agregadora para o dashboard /admin/diagnosticos.
-- SECURITY DEFINER + cheque explícito de papel admin (RLS contornado de forma controlada).
-- Retorna um único JSON com todos os blocos de métricas (Prompt 24).
...
BEGIN
  -- Bloqueio de acesso: só admin executa.
  IF NOT public.is_admin(auth.uid()) THEN
    RAISE EXCEPTION 'forbidden' USING ERRCODE = '42501';
  END IF;

  -- Conjunto-base: pacientes não-rascunho.
  ...
  cenarios_por_paciente AS (
    SELECT
      ...
      -- IG média no momento do diagnóstico, por momento.
  ...
  -- Foi para insulina: paciente teve cenario_3
  -- Só dieta: tem DMG e nunca iniciou insulina e nunca caiu em cenário 7.
  -- Insulina inicial OK: iniciou e ficou adequado (cenario_4) sem cair em cenário 7.
  resumo AS (
    SELECT
      count(*)                                                 AS total,
  ...
  -- Evolução mensal (últimos 12 meses) — gestantes cadastradas e diagnósticos confirmados.
  meses AS (
  ...
  -- Desfechos perinatais (apenas o que existe na tabela partos hoje).
  desfechos AS (
    SELECT
  ...
  SELECT jsonb_build_object(
    'resumo', jsonb_build_object(
      ...
      'taxa_controle_global',
        CASE WHEN v_total_dmg = 0 THEN 0
             ELSE round((v_total_dieta + v_total_insulina_ok)::numeric / v_total_dmg * 100, 1) END
    ),
    'evolucao_mensal', (
    ...
    'regional', jsonb_build_object(
      'por_estado', (
        ...
        FROM (
          SELECT pp.estado,
                 count(*) AS total,
                 count(*) FILTER (WHERE pp.tem_dmg) AS dmg
          FROM pp
          GROUP BY pp.estado
        ) t
      ),
      'por_cidade', (
        ...
        FROM (
          SELECT pp.cidade, pp.estado,
                 count(*) AS total,
                 count(*) FILTER (WHERE pp.tem_dmg) AS dmg
          FROM pp
          GROUP BY pp.cidade, pp.estado
          ORDER BY count(*) DESC
          LIMIT 20
        ) t
      ),
      'por_unidade', (
        ...
        FROM (
          SELECT pp.unidade_id,
                 count(*) AS total,
                 count(*) FILTER (WHERE pp.tem_dmg) AS dmg
          FROM pp
          WHERE pp.unidade_id IS NOT NULL
          GROUP BY pp.unidade_id
        ) t
