-- ============================================================
-- Quebras regionais: separar consultório × institucional.
--   • por_estado / por_cidade  → SÓ consultório (unidade_id IS NULL),
--     agrupado pela cidade/estado de RESIDÊNCIA da paciente.
--   • por_unidade              → institucional (unidade_id IS NOT NULL),
--     agrupado pela unidade de atendimento (inalterado).
-- Motivo: paciente institucional é atendida numa unidade fixa; agrupá-la
-- pela cidade de residência misturava as populações (ex.: hospital de SP
-- com pacientes "residentes" em várias cidades). Cada população passa a
-- aparecer na lente que faz sentido pra ela.
-- Também remove o piso "mín. 10 pacientes" da tabela de cidades (feito no
-- front): consultório acumula poucos pacientes por cidade.
-- Admin-only, SECURITY DEFINER. ⚠️ Aplicar à mão no Supabase (não roda no Publish).
-- ============================================================
    ...
    'regional', jsonb_build_object(
      ...
      'por_estado', (
        ...
        FROM (
          ...
          FROM pp WHERE pp.unidade_id IS NULL GROUP BY pp.estado
        ) t
      ),
      'por_cidade', (
        SELECT coalesce(jsonb_agg(jsonb_build_object(
        ...
        FROM (
          ...
          FROM pp WHERE pp.unidade_id IS NULL GROUP BY pp.cidade, pp.estado ORDER BY count(*) DESC LIMIT 20
        ) t
      ),
      -- por_unidade: institucional (unidade_id IS NOT NULL), pela unidade de atendimento.
      'por_unidade', (
        SELECT coalesce(jsonb_agg(jsonb_build_object(
